...
static image_list_t *create_gabor(int nscales, const int* or_, int width, int height)
{
    int i, j, fn;
    ...
    int nfilters = 0;
    for(i=0;i<nscales;i++)  nfilters+=or_[i];
    ...
    for(i = 1; i <= nscales; i++)
    {
        for(j = 1; j <= or_[i-1]; j++)
        {
            ...
            param[l][2] = 16*pow(or_[i-1], 2)/pow(32, 2);
            param[l][3] = M_PI/(or_[i-1])*(j-1);
            l++;
        }
    }
