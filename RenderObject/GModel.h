...
public:
	...
	GModel();
    void getVerticesAndFaces(std::vector<GLfloat> &vertice, std::vector<GLuint> &indices);
...
private:
    ...
    void getVerticesAndFacesRecursive(const aiScene *sc, const aiNode *nd, const glm::mat4 &inheritedTransformation, std::vector<GLfloat> &vertice, std::vector<GLuint> &indices);
